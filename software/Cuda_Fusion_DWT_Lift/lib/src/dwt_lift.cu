...
#else
    asm ("st.global.wt.v2.u16 [%0], {%1,%2};" :: "r"(a) , "h"(b), "h"(c));
#endif
...
#else
    asm ("st.global.wt.v2.u32 [%0], {%1,%2};" :: "r"(a) , "r"(b), "r"(c));
#endif
...
#else
    asm ("st.global.wt.v2.f32 [%0], {%1,%2};" :: "r"(a) , "f"(b), "f"(c));
#endif
#endif
#endif
}
...
static inline void DWT_F2(int DWT_Levels, int DSize_Initial_X, int DSize_Initial_Y, DATATYPE* DData_Initial, DATATYPE* DData_Final,cudaStream_t& stream)
{
    ...
    for(int Current_Level = DWT_Levels; Current_Level>0; --Current_Level)
    {
        ...
        Kernel_DWT_F<<<CUDA_number_blocks,NTHREADSBLOCK_DWT_F, SYNTHETIC_SHARED,stream>>>
                                                                                        (
                                                                                            DData_Initial_aux,
                                                                                            DData_Final,
                                                                                            DSize_Current_X,
...
static inline void DWT_R2(int DWT_Levels, int DSize_Initial_X, int DSize_Initial_Y, DATATYPE* DData_Initial, DATATYPE* DData_Final,cudaStream_t& stream)
{
...
dwt_lift::dwt_lift()
{
    ...
    cudaDeviceSetCacheConfig(cudaFuncCachePreferShared);
#else
    auto ok = cudaDeviceSetCacheConfig(cudaFuncCachePreferL1);
#endif
...
cudaError_t
dwt_lift::calc_forward(int DWT_Levels, int cols, int rows, DATATYPE *dev_in_image, DATATYPE *dev_out_image,cudaStream_t& stream) const
{
    DWT_F2(DWT_Levels, cols, rows,dev_in_image,dev_out_image,stream);
    return cudaError_t::cudaSuccess;
}

cudaError_t
dwt_lift::calc_inverse(int DWT_Levels, int cols, int rows, DATATYPE *dev_in_image, DATATYPE *dev_out_image, cudaStream_t &stream) const
{
    DWT_R2(DWT_Levels, cols, rows,dev_in_image,dev_out_image,stream);
    return cudaError_t::cudaSuccess;
}
